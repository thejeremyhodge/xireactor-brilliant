...
### Added
- **`[feature]` Sprint 0051 — personal zones** — every user gets an auto-created, walled-off **personal zone** that backs the "personal area" model envisioned for the contribution workflow. New additive migration `034_personal_zones.sql` adds `is_zone BOOLEAN` + `owner_user_id TEXT` to `groups`, a partial unique index `UNIQUE (org_id, owner_user_id) WHERE is_zone = TRUE`, a `provision_user_zone(p_user_id, p_org_id)` `SECURITY DEFINER` function, an `AFTER INSERT` trigger on `users` that calls it, a one-shot backfill loop for pre-existing users, and immutability triggers blocking non-`kb_admin` `UPDATE`/`DELETE` on zone groups or `INSERT`/`DELETE` on their `group_members` rows. Companion `SECURITY DEFINER` helper `grant_zone_admin_on_entry(...)` lets non-admin write paths grant the zone-admin permission without owning the `permissions` RLS write surface. (T-0301..T-0307; spec `.xireactor/specs/0051--2026-05-08--personal-zones-sprint-1.md`)
- **`[feature]` Default-write-to-zone safety invariant** — `POST /entries` with no `sensitivity` defaults to `'private'` and grants the caller's zone group `admin` on the new entry inside the same transaction as the entries INSERT (no half-states). Same zone grant fires for explicit `sensitivity='private'` (uniform rule). Callers passing explicit non-private sensitivity see no behavior change. New `api/services/zones.py::get_or_create_zone(conn, user_id, org_id)` is the resolver; `api/models.py::EntryCreate.sensitivity` is now optional with a `None` sentinel.
- **`[feature]` `GET /zone` + `POST /zone/promote` endpoints** — new `api/routes/zones.py` router. `GET /zone` returns the calling user's zone entries (limit/offset paginated). `POST /zone/promote` accepts `{entry_id, add_principals: [{principal_type, principal_id, role}], new_sensitivity?}`, INSERTs additive `permissions` rows (zone grant always preserved), optionally updates `entries.sensitivity`, and rejects promotion of entries not in the caller's zone. Promotion is **additive only** — the owner can never lose access to their own data through this surface.
- **`[feature]` MCP `list_zone` + `promote_entry` tools** — `mcp/tools.py` exposes both endpoints. `promote_entry` returns the updated entry plus a short human-readable summary of grants added so the LLM can confirm in chat. Both tools follow the existing tool-schema and structured-error patterns.
- **`[feature]` Skill v0.10.0 — Personal Zones (default safety) section** — SKILL.md teaches: any `create_entry` without explicit `sensitivity` lands in the caller's zone (walled off from everyone, including org admins via the API); `promote_entry` to share, `list_zone` to inspect; zone grant is permanent — promotion is additive. Worked example shows a viewer-grant + sensitivity bump in a single call. New `skill/_version.py` ships inside the bundle so consumers can introspect the version without parsing markdown frontmatter. Bundle re-zipped via `zip -FSr` (per `feedback_skill_rezip_macos_zip`).
- **`[feature]` API admin lock-down on zone groups** — `api/routes/groups.py` checks `is_zone` and 403s on `DELETE /groups/{id}`, `PATCH/PUT /groups/{id}` (rename / desc), and `POST /groups/{id}/members`. The migration-034 immutability triggers bypass for `kb_admin` (the role API admin requests run as), so the API-layer guard is the actual enforcement surface for org admins. Direct-DB superuser remains the legitimate maintenance escape hatch.

### Changed
- **`MIN_SKILL_VERSION` held at `0.7.0`** — personal zones are additive-only on the API surface (callers passing explicit sensitivity see no behavior change; the new endpoints + tools are net-new). v0.7.0 skills continue to work against the v0.10.0 API and receive the "newer skill available" banner via `LATEST_SKILL_VERSION`. `API_VERSION` / `MCP_VERSION` / `LATEST_SKILL_VERSION` bumped to `0.10.0`.

### Fixed
- **`[fix]` `provision_user_zone` EXECUTE grant for non-admin roles** (T-0301.1 — discovered by T-0305 test coverage) — migration 034 originally granted EXECUTE only to `kb_admin`, but `api/services/zones.py::get_or_create_zone` runs as the caller's PG role inside the request transaction. Non-admin users (`kb_editor` / `kb_commenter` / `kb_agent`) were hitting `InsufficientPrivilege` → 500 on every `POST /entries` without explicit sensitivity. Function is `SECURITY DEFINER`, so widening EXECUTE to all `kb_*` roles is bounded.
- **`[fix]` Zone-admin permission grant via `SECURITY DEFINER` helper** (T-0301.2 — discovered by T-0301.1 verification test) — after T-0301.1, the next statement in `create_entry` still failed: `INSERT INTO permissions ...` was denied because non-admin `kb_*` roles only have SELECT on `permissions` (and `permissions_admin_all` RLS is admin-only). Added narrowly-scoped `SECURITY DEFINER` helper `grant_zone_admin_on_entry(org_id, zone_group_id, entry_id, granted_by)` that only writes `principal_type='group' AND role='admin'` rows for zone groups. EXECUTE granted to all `kb_*` roles.
- **`[fix]` API admin can no longer mutate zone groups** (T-0303.1 — discovered by T-0305 test coverage) — `api/routes/groups.py` now 403s on delete / rename / desc-change / add-member / remove-member when the target group has `is_zone=TRUE`. The PG-role-`kb_admin` immutability bypass is closed at the API layer rather than retooling the trigger so the direct-DB superuser path remains a legitimate maintenance escape.
